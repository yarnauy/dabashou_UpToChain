...
contract ResourceCoinManager is Auth {
    ...
    event TransferEvidence(address indexed from, address indexed to, uint256 value, string evidence);
    ...
    function transferEvidence(TransferItem memory item) public {
        ...
        emit TransferEvidence(item.fromAddress, item.toAddress, item.amount, item.orderID);
    }
